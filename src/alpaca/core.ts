...
export const getDebtPoolBySymbol = (symbol: string) => ALL_POOLS.find(pool => {
  // debtIbWBNB -> debtIbBNB
  if (symbol === 'WBNB') symbol = 'BNB'
  return pool.stakingToken.toUpperCase().startsWith(`debtib${symbol}`.toUpperCase())
})
